...
function initWS(url, initMessage) {
    ...
    wss.on('connection', function connection(ws) {
        ws.on('message', function incoming(message) {
            ...
            // ws.send( broadcast.stack_is_empty ? 'Waiting for content to broadcast.' : broadcast.consume_stack()() )
        });
        console.log('broadcast stack length', broadcast.stack_length())
        broadcast.loop_method( (msg) => msg ? ws.send( msg ) : 'waiting...' );
    });
    // ******************
        ...
        try {
            ...
            switch (this_type) {
                ...
                case "transaction": {
                    ...
                    broadcast.add_to_stack( () => JSON.stringify(data))
                    break;
                }
                case "error": {
